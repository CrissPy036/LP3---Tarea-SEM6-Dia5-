...
class Articulo extends Conexion
{
    ...
    {
        $sql = "SELECT a.idArticulo,a.descripcion,a.preciocompra,a.precioventa,r.nombre AS
        rubro,a.idRubro FROM articulos a,rubros r WHERE a.idRubro = r.idRubro ORDER BY a.idArticulo
        DESC";
        $stmt = $this->conexion->prepare($sql);
        $stmt->execute();
    ...
    {
        $sql = "SELECT a.idArticulo,a.descripcion,a.preciocompra,a.precioventa,r.nombre AS
        rubro,a.idRubro FROM articulos a,rubros r WHERE a.idRubro = r.idRubro AND a.idArticulo = ?";
        $stmt = $this->conexion->prepare($sql);
    ...
    {
        $stmt = $this->conexion->prepare('UPDATE articulos SET descripcion = ?, preciocompra
        = ?,precioventa = ?, idRubro = ? WHERE idArticulo = ?');
